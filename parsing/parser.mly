...
/* $Id: parser.mly,v 1.7 1999/03/04 17:42:49 pessaux Exp $ */
...
let mkannotedtyp d =
  { patyp_desc = d; patyp_loc = symbol_loc() }
let mkpat d =
...
structure_item:
  ...
  | EXTERNAL val_ident COLON annoted_core_type opt_explaination EQUAL primitive_declaration
      { mkstr(Pstr_primitive($2, {paval_type = $4; paval_prim = $7;
				  paval_explaination = $5 })) }
...
annoted_core_type:
    simple_annoted_core_type
      { $1 }
  | annoted_core_type LESS phi_core_type MINUSGREATER annoted_core_type
      %prec prec_type_arrow
      { mkannotedtyp(Patyp_arrow($1, $3, $5)) }
  | annoted_core_type_tuple
      { mkannotedtyp(Patyp_tuple(List.rev $1)) }
;

simple_annoted_core_type:
    QUOTE ident
      { mkannotedtyp(Patyp_var $2) }
  | type_longident phi_core_type
      { mkannotedtyp(Patyp_constr($1, [], $2)) }
  | simple_annoted_core_type type_longident phi_core_type
      %prec prec_constr_appl
      { mkannotedtyp(Patyp_constr($2, [$1], $3)) }
  | LPAREN annoted_core_type_comma_list RPAREN type_longident phi_core_type
      %prec prec_constr_appl
      { mkannotedtyp (Patyp_constr($4, List.rev $2, $5)) }
  | LPAREN annoted_core_type RPAREN
      { $2 }
;
annoted_core_type_tuple:
    simple_annoted_core_type STAR simple_annoted_core_type      { [$3; $1] }
  | annoted_core_type_tuple STAR simple_annoted_core_type       { $3 :: $1 }
;
annoted_core_type_comma_list:
    annoted_core_type COMMA annoted_core_type                   { [$3; $1] }
  | annoted_core_type_comma_list COMMA annoted_core_type        { $3 :: $1 }
;

phi_core_type:
    LBRACKET UNDERSCORE RBRACKET
                           { Pann_bottom }
  | LBRACKET phi_elem_list BACKQUOTE ident RBRACKET
                           { Pann_explicit ($2, $4) }
;

phi_elem_list:
                                        { [] }
  | constant COLON core_presence SEMI phi_elem_list
          { (Pelem_constant ((Asttypes.string_of_constant $1), $3))
	    :: $5 }
  | ann_ident COLON core_presence SEMI phi_elem_list
          { (Pelem_constant ($1, $3)) :: $5 }
  | ann_ident LPAREN annoted_core_type RPAREN SEMI phi_elem_list
          { (Pelem_param ($1, $3)) :: $6 }
  | LBRACE annoted_label_declarations opt_semi RBRACE SEMI
          { [ Pelem_record $2 ] }
;
...
core_presence:
    ident       { if $1 = "Pre" then Prexpr_pre else Prexpr_var $1 }
;

annoted_label_declarations:
    annoted_label_declaration
          { [$1] }
  | annoted_label_declarations SEMI annoted_label_declaration
          { $3 :: $1 }
;
annoted_label_declaration:
    mutable_flag LIDENT COLON annoted_core_type    { ($2, $4) }
;

opt_explaination:
                                               { [] }
  | WITH annoted_type_explaination annoted_type_explaination_list
           { $2 :: $3 }
;

annoted_type_explaination:
  | QUOTE ident EQUAL annoted_core_type      { ($2, $4) }
;

annoted_type_explaination_list:
                       { [] }
  | AND annoted_type_explaination annoted_type_explaination_list
                       { $2 :: $3 }
;
...
constant:
    INT                                   { Const_int $1 }
  | CHAR                                  { Const_char $1 }
  | STRING                                { Const_string (String.escaped $1) }
  | FLOAT                                 { Const_float $1 }
;
signed_constant:
    constant                                    { $1 }
...
mod_ext_longident:
  ...
  | mod_ext_longident LPAREN mod_ext_longident RPAREN
      { failwith "Lapply idents not supported" }
;
mty_longident:
    ident                                       { Lident $1 }
