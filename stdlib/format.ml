...
(* $Id: format.ml,v 1.3 1998/12/08 16:13:01 pessaux Exp $ *)
...
(* !!! Contains non genralized effect *)
let std_formatter =
...
(* !!! Eta expansions *)
(* !!! Protection *)
let open_hbox x = try pp_open_hbox std_formatter x with _ -> failwith "FORMAT"
and open_vbox x = try pp_open_vbox std_formatter x with _ -> failwith "FORMAT"
and open_hvbox x =
  try pp_open_hvbox std_formatter x with _ -> failwith "FORMAT"
and open_hovbox x =
  try pp_open_hovbox std_formatter x with _ -> failwith "FORMAT"
and open_box x = try pp_open_box std_formatter x with _ -> failwith "FORMAT"
and close_box x = try pp_close_box std_formatter x with _ -> failwith "FORMAT"
and print_as x y =
  try pp_print_as std_formatter x y with _ -> failwith "FORMAT"
and print_string x =
  try pp_print_string std_formatter x with _ -> failwith "FORMAT"
and print_int x = try pp_print_int std_formatter x with _ -> failwith "FORMAT"
and print_float x =
  try pp_print_float std_formatter x with _ -> failwith "FORMAT"
and print_char x =
  try pp_print_char std_formatter x with _ -> failwith "FORMAT"
and print_bool x =
  try pp_print_bool std_formatter x with _ -> failwith "FORMAT"
and print_break x y =
  try pp_print_break std_formatter x y with _ -> failwith "FORMAT"
and print_cut x = try pp_print_cut std_formatter x with _ -> failwith "FORMAT"
and print_space x =
  try pp_print_space std_formatter x with _ -> failwith "FORMAT"
and force_newline x =
  try pp_force_newline std_formatter x with _ -> failwith "FORMAT"
and print_flush x = try pp_print_flush std_formatter x with _ -> failwith "FORMAT"
and print_newline x =
  try pp_print_newline std_formatter x with _ -> failwith "FORMAT"
and print_if_newline x =
  try pp_print_if_newline std_formatter x with _ -> failwith "FORMAT"
and open_tbox x = try pp_open_tbox std_formatter x with _ -> failwith "FORMAT"
and close_tbox x =
  try pp_close_tbox std_formatter x with _ -> failwith "FORMAT"
and print_tbreak x y =
  try pp_print_tbreak std_formatter x y with _ -> failwith "FORMAT"
and set_tab x = try pp_set_tab std_formatter x with _ -> failwith "FORMAT"
and print_tab x = try pp_print_tab std_formatter x with _ -> failwith "FORMAT"
and set_margin x =
  try pp_set_margin std_formatter x with _ -> failwith "FORMAT"
and get_margin x =
  try pp_get_margin std_formatter x with _ -> failwith "FORMAT"
and set_max_indent x =
  try pp_set_max_indent std_formatter x with _ -> failwith "FORMAT"
and get_max_indent x =
  try pp_get_max_indent std_formatter x with _ -> failwith "FORMAT"
and set_max_boxes x =
  try pp_set_max_boxes std_formatter x with _ -> failwith "FORMAT"
and get_max_boxes x =
  try pp_get_max_boxes std_formatter x with _ -> failwith "FORMAT"
and over_max_boxes x =
  try pp_over_max_boxes std_formatter x with _ -> failwith "FORMAT"
and set_ellipsis_text x =
  try pp_set_ellipsis_text std_formatter x with _ -> failwith "FORMAT"
and get_ellipsis_text x =
  try pp_get_ellipsis_text std_formatter x with _ -> failwith "FORMAT"
and set_formatter_out_channel x =
    try pp_set_formatter_out_channel std_formatter x
    with _ -> failwith "FORMAT"
(* !!! Contains non genralized effect *)
and set_formatter_output_functions x y =
    try pp_set_formatter_output_functions std_formatter x y
    with _ -> failwith "FORMAT"
(* !!! Contains non genralized effect *)
and get_formatter_output_functions x =
    try pp_get_formatter_output_functions std_formatter x
    with _ -> failwith "FORMAT" ;;

external format_int: string[`a] <[`b]-> int[`c] <[`d]->
                     string[_] = "format_int" ;;
external format_float: string[`a] <[`b]-> float[`c] <[`d]->
                     string[_] = "format_float" ;;
